...
public class Main {
    ...
    void solve() throws Exception {
        int t = ni(); // Read number of test cases
        while (t-- > 0) {
            int n = ni();
            ...
            // 1. Each '0' forms its own valid subsequence
            ans += count0;
            
            // 2. Combine one '1' and one '2' to sum to 3
            int pairs = Math.min(count1, count2);
            ans += pairs;
            count1 -= pairs;
            count2 -= pairs;
            
            // 3. Combine remaining identical elements into triplets (1+1+1=3 or 2+2+2=6)
            ans += count1 / 3;
            ans += count2 / 3;
            
            pn(ans); // Print result using template's method
        }
    }
    ...
    void p(Object... o) {
        for (Object oo : o) out.print(oo);
    }
 
    void pn(Object... o) {
        for (int i = 0; i < o.length; i++) out.print(o[i] + (i + 1 < o.length ? " " : "\n"));
    }
